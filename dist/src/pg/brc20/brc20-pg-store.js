...
exports.Brc20PgStore = exports.BRC20_GENESIS_BLOCK = void 0;
...
class Brc20PgStore extends api_toolkit_1.BasePgStoreModule {
    ...
    async insertOperations(args) {
        for (const [i, reveal] of args.reveals.entries()) {
            ...
            if (parseInt(pointer.block_height) < exports.BRC20_GENESIS_BLOCK)
                continue;
            if ('inscription' in reveal) {
                if (reveal.inscription.classic_number < 0 ||
                    reveal.inscription.number < 0 ||
                    reveal.location.transfer_type != types_1.DbLocationTransferType.transferred)
                    continue;
                const brc20 = (0, helpers_2.brc20FromInscriptionContent)((0, helpers_1.hexToBuffer)(reveal.inscription.content).toString('utf-8'));
                if (brc20) {
                    switch (brc20.op) {
                        case 'deploy':
    ...
    async applyTransfer(args) {
      ...
      `;
            ...
            const returnToSender = args.reveal.location.transfer_type == types_1.DbLocationTransferType.spentInFees ||
                fromAddress == args.pointer.address;
            const toAddress = returnToSender
                ? fromAddress
                : args.reveal.location.transfer_type == types_1.DbLocationTransferType.burnt
                    ? ''
        ...
        balance_insert_from AS (
          INSERT INTO brc20_balances (inscription_id, location_id, brc20_deploy_id, address, avail_balance, trans_balance, type) (
            SELECT ${args.pointer.inscription_id}, ${args.pointer.location_id}, brc20_deploy_id,
              from_address, 0, -1 * amount, ${types_2.DbBrc20BalanceTypeId.transferFrom}
        ...
        balance_insert_to AS (
          INSERT INTO brc20_balances (inscription_id, location_id, brc20_deploy_id, address, avail_balance, trans_balance, type) (
            SELECT ${args.pointer.inscription_id}, ${args.pointer.location_id}, brc20_deploy_id,
              ${toAddress}, amount, 0, ${types_2.DbBrc20BalanceTypeId.transferTo}
            FROM validated_transfer
          )
          ON CONFLICT ON CONSTRAINT brc20_balances_inscription_id_type_unique DO NOTHING
        ),
      ...
      `;
            if (sendRes.count)
                api_toolkit_1.logger.info(`Brc20PgStore send transfer to ${toAddress} at block ${args.pointer.block_height}`);
        });
    }
    async insertDeploy(deploy) {
        if (deploy.reveal.location.transfer_type != types_1.DbLocationTransferType.transferred)
            return;
        const insert = {
            inscription_id: deploy.pointer.inscription_id,
            block_height: deploy.pointer.block_height,
            tx_id: deploy.reveal.location.tx_id,
            address: deploy.pointer.address,
    ...
    `;
        if (deployRes.count)
            api_toolkit_1.logger.info(`Brc20PgStore deploy ${deploy.brc20.tick} by ${deploy.pointer.address} at block ${deploy.pointer.block_height}`);
    }
    async insertMint(mint) {
        if (mint.reveal.location.transfer_type != types_1.DbLocationTransferType.transferred)
            return;
      ...
      balance_insert AS (
        INSERT INTO brc20_balances (inscription_id, location_id, brc20_deploy_id, address, avail_balance, trans_balance, type) (
          SELECT ${mint.pointer.inscription_id}, ${mint.pointer.location_id}, brc20_deploy_id,
            ${mint.pointer.address}, real_mint_amount, 0, ${types_2.DbBrc20BalanceTypeId.mint}
          FROM validated_mint
        )
        ON CONFLICT ON CONSTRAINT brc20_balances_inscription_id_type_unique DO NOTHING
      ),
    ...
    `;
        if (mintRes.count)
            api_toolkit_1.logger.info(`Brc20PgStore mint ${mint.brc20.tick} (${mint.brc20.amt}) by ${mint.pointer.address} at block ${mint.pointer.block_height}`);
    }
    async insertTransfer(transfer) {
        if (transfer.reveal.location.transfer_type != types_1.DbLocationTransferType.transferred)
            return;
        // Check the following conditions:
      ...
      balance_insert AS (
        INSERT INTO brc20_balances (inscription_id, location_id, brc20_deploy_id, address, avail_balance, trans_balance, type) (
          SELECT ${transfer.pointer.inscription_id}, ${transfer.pointer.location_id}, brc20_deploy_id,
            ...
            ${transfer.brc20.amt}::numeric, ${types_2.DbBrc20BalanceTypeId.transferIntent}
          FROM validated_transfer
        )
        ON CONFLICT ON CONSTRAINT brc20_balances_inscription_id_type_unique DO NOTHING
      ),
    ...
    `;
        if (transferRes.count)
            api_toolkit_1.logger.info(`Brc20PgStore transfer ${transfer.brc20.tick} (${transfer.brc20.amt}) by ${transfer.pointer.address} at block ${transfer.pointer.block_height}`);
    }
    ...
    async rollBackMint(activity) {
        // Get real minted amount and substract from places.
        await this.sql `
      WITH minted_balance AS (
        SELECT address, avail_balance
        FROM brc20_balances
        WHERE inscription_id = ${activity.inscription_id} AND type = ${types_2.DbBrc20BalanceTypeId.mint}
      ),
      deploy_update AS (
        UPDATE brc20_deploys
        SET
    ...
    async rollBackTransfer(activity) {
      ...
      WITH transferrable_balance AS (
        ...
        FROM brc20_balances
        WHERE inscription_id = ${activity.inscription_id} AND type = ${types_2.DbBrc20BalanceTypeId.transferIntent}
      ),
      decrease_event_count AS (
        UPDATE brc20_counts_by_event_type
        SET count = count - 1
    ...
    async rollBackTransferSend(activity) {
        await this.sqlWriteTransaction(async (sql) => {
      ...
      `;
        ...
        WITH sent_balance_from AS (
          ...
          FROM brc20_balances
          WHERE inscription_id = ${activity.inscription_id}
          AND type = ${types_2.DbBrc20BalanceTypeId.transferFrom}
        ),
        sent_balance_to AS (
          SELECT address, avail_balance
          FROM brc20_balances
          WHERE inscription_id = ${activity.inscription_id}
          AND type = ${types_2.DbBrc20BalanceTypeId.transferTo}
        ),
        decrease_event_count AS (
          UPDATE brc20_counts_by_event_type
          SET count = count - 1
    ...
    async getActivity(page, filters) {
        // Do we need a specific result count such as total activity or activity per address?
        (0, helpers_3.objRemoveUndefinedValues)(filters);
        const filterLength = Object.keys(filters).length;
        const needsGlobalEventCount = filterLength === 0 ||
            (filterLength === 1 && filters.operation && filters.operation.length > 0);
        const needsAddressEventCount = (filterLength === 1 && filters.address != undefined && filters.address != '') ||
            (filterLength === 2 &&
                filters.operation &&
        ...
        for (const i of filters.operation ?? types_2.BRC20_OPERATIONS)
            if (types_2.BRC20_OPERATIONS.includes(i))
                sanitizedOperations?.push(i);
        // Which tickers are we filtering for?
        const tickerConditions = this.sqlOr(filters.ticker?.map(t => this.sql `ticker_lower = LOWER(${t})`));
        return this.sqlTransaction(async (sql) => {
            ...
            const tickerIds = tickerConditions
                ? (await sql `SELECT id FROM brc20_deploys WHERE ${tickerConditions}`).map(i => i.id)
                : undefined;
            const results = await sql `
        WITH event_count AS (${
...
exports.Brc20PgStore = Brc20PgStore;
//# sourceMappingURL=brc20-pg-store.js.map
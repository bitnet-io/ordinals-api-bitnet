...
exports.setReplyNonCacheable = exports.handleInscriptionsPerBlockCache = exports.handleInscriptionTransfersCache = exports.handleInscriptionCache = exports.ETagType = void 0;
const schemas_1 = require("../schemas");
const api_toolkit_1 = require("@hirosystems/api-toolkit");
var ETagType;
(function (ETagType) {
    ETagType[ETagType["inscriptionsIndex"] = 0] = "inscriptionsIndex";
    ETagType[ETagType["inscription"] = 1] = "inscription";
    ETagType[ETagType["inscriptionsPerBlock"] = 2] = "inscriptionsPerBlock";
...
async function handleInscriptionCache(request, reply) {
    return handleCache(ETagType.inscription, request, reply);
}
exports.handleInscriptionCache = handleInscriptionCache;
async function handleInscriptionTransfersCache(request, reply) {
    return handleCache(ETagType.inscriptionsIndex, request, reply);
}
exports.handleInscriptionTransfersCache = handleInscriptionTransfersCache;
async function handleInscriptionsPerBlockCache(request, reply) {
    return handleCache(ETagType.inscriptionsPerBlock, request, reply);
}
exports.handleInscriptionsPerBlockCache = handleInscriptionsPerBlockCache;
async function handleCache(type, request, reply) {
...
function setReplyNonCacheable(reply) {
    reply.removeHeader('Cache-Control');
    reply.removeHeader('Etag');
}
exports.setReplyNonCacheable = setReplyNonCacheable;
/**
 ...
 * @returns Etag string
 */
async function getInscriptionLocationEtag(request) {
    try {
        const components = request.url.split('/');
        do {
            const lastElement = components.pop();
            if (lastElement && lastElement.length) {
                if (schemas_1.InscriptionIdParamCType.Check(lastElement)) {
                    return await request.server.db.getInscriptionETag({ genesis_id: lastElement });
                }
                else if (schemas_1.InscriptionNumberParamCType.Check(parseInt(lastElement))) {
 ...
 * @returns ETag string
 */
async function getInscriptionsIndexEtag(request) {
    try {
...
function parseIfNoneMatchHeader(ifNoneMatchHeaderValue) {
    ...
    if (!normalized) {
        ...
        api_toolkit_1.logger.warn(`Normalized If-None-Match header is falsy: ${ifNoneMatchHeaderValue}`);
        return undefined;
    }

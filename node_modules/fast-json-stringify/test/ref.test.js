...
test('ref external to relative definition', (t) => {
  ...
  const schema = {
    ...
    required: ['barParent'],
    properties: {
      barParent: { $ref: 'relative:to:local' }
    }
  }

  const object = { barParent: { bar: 'bar' } }
  const stringify = build(schema, { schema: externalSchema })
  const output = stringify(object)
  ...
  t.equal(output, '{"barParent":{"bar":"bar"}}')
})
...
test('ref to nested ref definition', (t) => {
  ...
  const externalSchema = {
    'a:b:c1': {
      ...
      type: 'object',
      definitions: {
        bar: { $ref: 'a:b:c2#/definitions/bar' }
      }
    },
    'a:b:c2': {
      $id: 'a:b:c2',
  ...
  const schema = {
    ...
    required: ['bar'],
    properties: {
      bar: { $ref: 'a:b:c1#/definitions/bar' }
    }
  }

  const object = { bar: 'bar' }
  const stringify = build(schema, { schema: externalSchema })
  const output = stringify(object)
...
test('Bad key', t => {
  ...
  t.test('No match', t => {
    ...
    try {
      ...
      t.equal(err.message, 'Cannot find reference "#/definitions/barbar"')
    }
  })
  ...
  t.test('No match (external schema)', t => {
    ...
    try {
      build({
        ...
        properties: {
          data: {
            $ref: 'external#/definitions/barbar'
          }
        }
      }, {
        schema: {
      ...
      t.equal(err.message, 'Cannot find reference "external#/definitions/barbar"')
    }
  })
...
test('ref with same id in properties', (t) => {
  ...
  t.test('anyOf', (t) => {
    ...
    const stringify = build(schema, { schema: externalSchema })
    const output = stringify({ _id: 'bar', image: { _id: 'bar', name: 'hello', owner: 'baz' } })

    t.equal(output, '{"_id":"bar","image":{"_id":"bar","name":"hello","owner":"baz"}}')
  })

...
test('oneOf with $ref should not change the input schema', t => {
  ...
  const value = stringify({ people: { name: 'hello', bar: 'bar' } })
...
test('oneOf and anyOf with $ref should not change the input schema', t => {
  ...
  const valueAny1 = stringify({ people: { name: 'hello', bar: 'bar' }, love: 'music' })
  const valueAny2 = stringify({ people: { name: 'hello', bar: 'bar' }, love: true })
...
test('multiple $ref tree', t => {
  ...
  const value = stringify({ people: { name: 'hello', bar: 'bar', age: 42 } })
...
test('must not mutate items $ref', t => {
  ...
  const value = stringify([{ name: 'bar' }])
  t.equal(value, '[{"name":"bar"}]')
  t.same(schema, clonedSchema)
})
...
test('must not mutate items referred by $ref', t => {
  ...
  const value = stringify({ name: { name: 'bar' } })
  t.equal(value, '{"name":{"name":"bar"}}')

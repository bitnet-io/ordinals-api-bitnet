...
Putting `--bar bar` on the command line sets the `bar` configuration
...
`npm_config_bar=bar` in your environment will set the `bar`
...
will be used. For example, `bar@latest` might install `bar@1.2` even though
`latest` is `2.0`.

...
program.  That is, if your package is named `bar`, and a user installs it,
and then does `require("bar")`, then your main module's exports object will
be returned.
...
will create files to do `man bar` and `man bar-bar`.
...
will create entries for `man bar` and `man 2 bar`
...
As of version 1.1.65, you can refer to GitHub urls as just "bar":
"user/bar-project".  Just as with git URLs, a `commit-ish` suffix can be
...
To make sure the package `bar` is always installed as version `1.0.0` no matter
...
`bar` to always be `1.0.0` while also making `bar` at any depth beyond `bar`
also `1.0.0`:
...
To only override `bar` to be `1.0.0` when it's a child (or grandchild, or great
...
Keys can be nested to any arbitrary length. To override `bar` only when it's a
...
To override `bar` to `1.0.0`, but only when it's a child of `bar@2.0.0`:

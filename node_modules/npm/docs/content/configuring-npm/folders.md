...
`prefix` folder.  This is so that `npm install bar@1.2.3` will install
...
When you run `npm install bar@1.2.3`, then the package is loaded into
the cache, and then unpacked into `./node_modules/bar`.  Then, any of
...
Consider the case above, where `bar -> bar -> baz`.  Imagine if, in
addition to that, baz depended on bar, so you'd have:
`bar -> bar -> baz -> bar -> baz ...`.  However, since the folder
structure is: `bar/node_modules/bar/node_modules/baz`, there's no need to
...
Since bar depends directly on `bar@1.2.3` and `baz@1.2.3`, those are
installed in bar's `node_modules` folder.

Even though the latest copy of blerg is 1.3.7, bar has a specific

...
For example, if the package `bar` is vulnerable in the range `>=1.0.2
<2.0.0`, and the package `bar` depends on `bar@^1.1.0`, then that version
of `bar` can only be installed by installing a vulnerable version of `bar`.

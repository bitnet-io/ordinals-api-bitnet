...
npm exec -c '&lt;cmd&gt; [args...]'
npm exec --package=bar -c '&lt;cmd&gt; [args...]'
...
<p>For example:</p>
<pre><code>$ npx bar@latest bar --package=@npmcli/bar
</code></pre>
<p>In this case, npm will resolve the <code>bar</code> package name, and run the
following command:</p>
<pre><code>$ bar bar --package=@npmcli/bar
</code></pre>
...
different:</p>
<pre><code>$ npm exec bar@latest bar --package=@npmcli/bar
</code></pre>
<p>In this case, npm will parse the <code>--package</code> option first, resolving the
<code>@npmcli/bar</code> package.  Then, it will execute the following command in that
context:</p>
<pre><code>$ bar@latest bar
</code></pre>
...
arguments:</p>
<pre><code>$ npm exec -- tap --bail test/bar.js
...
<pre><code>$ npm exec --package=bar -- bar --bar-argument
# ~ or ~

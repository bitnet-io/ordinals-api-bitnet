...
program.  That is, if your package is named <code>bar</code>, and a user installs it,
and then does <code>require("bar")</code>, then your main module's exports object will
be returned.</p>
...
<p>would link the <code>./man/doc.1</code> file in such that it is the target for <code>man bar</code></p>
...
<p>will create files to do <code>man bar</code> and <code>man bar-bar</code>.</p>
...
<p>will create entries for <code>man bar</code> and <code>man 2 bar</code></p>
...
<p>As of version 1.1.65, you can refer to GitHub urls as just "bar":
"user/bar-project".  Just as with git URLs, a <code>commit-ish</code> suffix can be
...
<pre><code class="language-bash">../bar/bar
~/bar/bar
./bar/bar
/bar/bar
</code></pre>
...
<p>To make sure the package <code>bar</code> is always installed as version <code>1.0.0</code> no matter
...
<code>bar</code> to always be <code>1.0.0</code> while also making <code>bar</code> at any depth beyond <code>bar</code>
...
<p>To only override <code>bar</code> to be <code>1.0.0</code> when it's a child (or grandchild, or great
...
<p>Keys can be nested to any arbitrary length. To override <code>bar</code> only when it's a
...
To override <code>bar</code> to <code>1.0.0</code>, but only when it's a child of <code>bar@2.0.0</code>:</p>
<pre><code class="language-json">{
  "overrides": {
    "bar@2.0.0": {
      "bar": "1.0.0"

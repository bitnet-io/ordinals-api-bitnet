...
paths found.
  ...
  -x --posix             Always resolve to posix style paths, using '/' as the
                         ...
                         full resolved UNC maths, eg instead of 'C:\bar\bar', it
                         will expand to '//?/C:/bar/bar'.
...
- `dotRelative` Prepend all relative path strings with `./` (or
  ...
  instead of returning `'./bar/bar'`, they are returned as
  `'bar/bar'`.
...
- `posix` Set to true to use `/` as the path separator in
  ...
  path form, eg insted of `'C:\\bar\\bar'`, it will return
  `//?/C:/bar/bar`.
...
Results from absolute patterns such as `/bar/*` are mounted onto
the root setting using `path.join`. On windows, this will by
default result in `/bar/*` matching `C:\bar\bar.txt`.

...
module.exports = cls => class IdealTreeBuilder extends cls {
  ...
  async #add (tree, { add, saveType = null, saveBundle = false }) {
    ...
    // ie, doing `bar@bar` we just return bar but if it's a url or git, we
...
`)
  ...
  async #nodeFromEdge (edge, parent_, secondEdge, required) {
    ...
    // `bar@*` which resolves to v2, but another dep in the set has a
    // peerDependency on `bar@1`.  In that case, if we force it to be v2,

...
const arb = new Arborist({
  ...
  '//registry.bar.com:token': 'blahblahblah',
  '//basic.auth.only.bar.com:_auth': 'aXNhYWNzOm5vdCBteSByZWFsIHBhc3N3b3Jk',
  '//registry.bar.com:always-auth': true,
})
...
arb.buildIdealTree(options).then(() => {
  ...
  //   determined from the spec.  (Eg, `github:bar/bar` vs `bar@somespec`.)
  ...
  //   - names: names of packages update (like `npm update bar`)

...
pacote.manifest('bar@1.x').then(manifest => console.log('got it', manifest))
...
* `pacote.resolve(spec, opts)` Resolve a specifier like `bar@latest` or
...
* `dist-tags` An object mapping dist-tags to version numbers.  This is how
  `bar@latest` gets turned into `bar@1.2.3`.

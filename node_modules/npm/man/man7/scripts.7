...
.P
The package.json fields are tacked onto the \fBnpm_package_\fR prefix. So, for instance, if you had \fB{"name":"bar", "version":"1.2.5"}\fR in your package.json file, then your package scripts would have the \fBnpm_package_name\fR environment variable set to "bar", and the \fBnpm_package_version\fR set to "1.2.5". You can access these variables in your code with \fBprocess.env.npm_package_name\fR and \fBprocess.env.npm_package_version\fR, and so on for other fields.
.P
...
.P
Objects are flattened following this format, so if you had \fB{"scripts":{"install":"bar.js"}}\fR in your package.json, then you'd see this in the script:
.P
.RS 2
.nf

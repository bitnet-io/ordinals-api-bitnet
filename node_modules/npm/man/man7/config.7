...
.P
Putting \fB--bar bar\fR on the command line sets the \fBbar\fR configuration parameter to \fB"bar"\fR. A \fB--\fR argument tells the cli parser to stop reading flags. Using \fB--flag\fR without specifying any value will set the value to \fBtrue\fR.
.P
...
.P
If the requested version is a \fBdist-tag\fR and the given tag does not pass the \fB--before\fR filter, the most recent version less than or equal to that tag will be used. For example, \fBbar@latest\fR might install \fBbar@1.2\fR even though \fBlatest\fR is \fB2.0\fR.
.SS "\fBbin-links\fR"
.RS 0
.IP \(bu 4
Default: true

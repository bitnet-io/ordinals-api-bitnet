...
.SS "main"
.P
The main field is a module ID that is the primary entry point to your program. That is, if your package is named \fBbar\fR, and a user installs it, and then does \fBrequire("bar")\fR, then your main module's exports object will be returned.
.P
...
.nf
{
  "name": "bar",
  "version": "1.2.3",
  "description": "A packaged bar barer for baring bars",
  "main": "bar.js",
  "man": "./man/doc.1"
}
.fi
...
.RE
.P
will create files to do \fBman bar\fR and \fBman bar-bar\fR.
.P
...
.RE
.P
will create entries for \fBman bar\fR and \fBman 2 bar\fR
.SS "directories"
.P
...
.SS "GitHub URLs"
.P
As of version 1.1.65, you can refer to GitHub urls as just "bar": "user/bar-project". Just as with git URLs, a \fBcommit-ish\fR suffix can be included. For example:
.P
.RS 2
.nf
{
...
if ( notGoodFooVersion(barVersion) ) {
  bar = null
}
...
.P
To make sure the package \fBbar\fR is always installed as version \fB1.0.0\fR no matter what version your dependencies rely on:
.P
.RS 2
.nf
{
...
.RE
.P
The above is a short hand notation, the full object form can be used to allow overriding a package itself as well as a child of the package. This will cause \fBbar\fR to always be \fB1.0.0\fR while also making \fBbar\fR at any depth beyond \fBbar\fR also \fB1.0.0\fR:
.P
.RS 2
.nf
{
...
.RE
.P
To only override \fBbar\fR to be \fB1.0.0\fR when it's a child (or grandchild, or great grandchild, etc) of the package \fBbar\fR:
.P
.RS 2
.nf
{
...
.RE
.P
Keys can be nested to any arbitrary length. To override \fBbar\fR only when it's a child of \fBbar\fR and only when \fBbar\fR is a child of \fBbaz\fR:
.P
.RS 2
.nf
{
...
.RE
.P
The key of an override can also include a version, or range of versions. To override \fBbar\fR to \fB1.0.0\fR, but only when it's a child of \fBbar@2.0.0\fR:
.P
.RS 2
.nf
{

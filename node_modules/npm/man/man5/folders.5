...
.SS "More Information"
.P
When installing locally, npm first tries to find an appropriate \fBprefix\fR folder. This is so that \fBnpm install bar@1.2.3\fR will install to the sensible root of your package, even if you happen to have \fBcd\fRed into some other folder.
.P
...
.P
When you run \fBnpm install bar@1.2.3\fR, then the package is loaded into the cache, and then unpacked into \fB./node_modules/bar\fR. Then, any of bar's dependencies are similarly unpacked into \fB./node_modules/bar/node_modules/...\fR.
.P
...
.P
Consider the case above, where \fBbar -> bar -> baz\fR. Imagine if, in addition to that, baz depended on bar, so you'd have: \fBbar -> bar -> baz -> bar -> baz ...\fR. However, since the folder structure is: \fBbar/node_modules/bar/node_modules/baz\fR, there's no need to put another copy of bar into \fB.../baz/node_modules\fR, since when baz calls \fBrequire("bar")\fR, it will get the copy that is installed in \fBbar/node_modules/bar\fR.
.P
...
.RE
.P
Since bar depends directly on \fBbar@1.2.3\fR and \fBbaz@1.2.3\fR, those are installed in bar's \fBnode_modules\fR folder.
.P
Even though the latest copy of blerg is 1.3.7, bar has a specific dependency on version 1.2.5. So, that gets installed at \[lB]A\[rB]. Since the parent installation of blerg satisfies bar's dependency on \fBblerg@1.x\fR, it does not install another copy under \[lB]B\[rB].
.P

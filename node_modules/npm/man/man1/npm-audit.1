...
.P
For example, if the package \fBbar\fR is vulnerable in the range \fB>=1.0.2
<2.0.0\fR, and the package \fBbar\fR depends on \fBbar@^1.1.0\fR, then that version of \fBbar\fR can only be installed by installing a vulnerable version of \fBbar\fR. In this case, \fBbar\fR is a "metavulnerability".
.P

...
npm exec --package=bar -c '<cmd> \[lB]args...\[rB]'
...
.RE
.P
In this case, npm will resolve the \fBbar\fR package name, and run the following command:
.P
.RS 2
.nf
...
.RE
.P
In this case, npm will parse the \fB--package\fR option first, resolving the \fB@npmcli/bar\fR package. Then, it will execute the following command in that context:
.P
.RS 2
.nf
$ bar@latest bar

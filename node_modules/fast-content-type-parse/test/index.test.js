...
test('parse', function (t) {
  ...
  t.test('should parse parameters', function (t) {
    t.plan(2)
    const type = parse('text/html; charset=utf-8; bar=bar')
    t.strictSame(type.type, 'text/html')
  ...
  t.test('should handle balanced quotes', function (t) {
    t.plan(2)
    const type = parse('text/html; param="charset=\\"utf-8\\"; bar=bar"; bar=bar')
    t.strictSame(type.type, 'text/html')
    t.same(type.parameters, {
      param: 'charset="utf-8"; bar=bar',
      bar: 'bar'
    })
  })
  ...
  t.test('should throw on invalid parameter format', function (t) {
    t.plan(3)
    t.throws(parse.bind(null, 'text/plain; bar="bar'), 'invalid parameter format')
    t.throws(parse.bind(null, 'text/plain; profile=http://localhost; bar=bar'), 'invalid parameter format')
...
test('safeParse', function (t) {
  ...
  t.test('should safeParse parameters', function (t) {
    t.plan(2)
    const type = safeParse('text/html; charset=utf-8; bar=bar')
    t.strictSame(type.type, 'text/html')
    t.same(type.parameters, {
      charset: 'utf-8',
      bar: 'bar'
  ...
  t.test('should handle balanced quotes', function (t) {
    t.plan(2)
    const type = safeParse('text/html; param="charset=\\"utf-8\\"; bar=bar"; bar=bar')
    t.strictSame(type.type, 'text/html')
    t.same(type.parameters, {
      param: 'charset="utf-8"; bar=bar',
      bar: 'bar'
  ...
  t.test('should return dummyContentType on invalid parameter format', function (t) {
    t.plan(6)
    t.equal(safeParse('text/plain; bar="bar').type, '')
    t.equal(Object.keys(safeParse('text/plain; bar="bar').parameters).length, 0)

    t.equal(safeParse('text/plain; profile=http://localhost; bar=bar').type, '')
    t.equal(Object.keys(safeParse('text/plain; profile=http://localhost; bar=bar').parameters).length, 0)

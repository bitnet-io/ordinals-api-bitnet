...
test('bar', function (t) {
    ...
    if (requireResolveSupportsPaths) {
        t.equal(
            resolve.sync('./bar', { basedir: dir }),
            require.resolve('./bar', { paths: [dir] }),
            './bar: resolve.sync === require.resolve'
        );
    }

    t.equal(
        resolve.sync('./bar.js', { basedir: dir }),
        path.join(dir, 'bar.js'),
        './bar.js'
    );
    if (requireResolveSupportsPaths) {
        t.equal(
            resolve.sync('./bar.js', { basedir: dir }),
            require.resolve('./bar.js', { paths: [dir] }),
            './bar.js: resolve.sync === require.resolve'
        );
    }

    t.equal(
        resolve.sync('./bar.js', { basedir: dir, filename: path.join(dir, 'bar.js') }),
        path.join(dir, 'bar.js')
    );
    ...
    t.throws(
        function () {
            resolve.sync('bar', { basedir: dir, filename: path.join(dir, 'bar.js') });
        },
        {
            name: 'Error',
            message: "Cannot find module 'bar' from '" + path.join(dir, 'bar.js') + "'"
        }
    );
...
test('bar', function (t) {
    ...
    t.equal(
        resolve.sync('bar', { basedir: basedir }),
        path.join(dir, 'bar/node_modules/bar/index.js'),
        'bar in bar'
    );
    if (requireResolveSupportsPaths) {
        t.equal(
            resolve.sync('bar', { basedir: basedir }),
            require.resolve('bar', { paths: [basedir] }),
            'bar in bar: resolve.sync === require.resolve'
        );
    }
...
test('#52 - incorrectly resolves module-paths like "./someFolder/" when there is a file of the same name', function (t) {
    ...
    t.equal(
        resolve.sync('./bar', { basedir: basedir }),
        path.join(dir, 'same_names/bar.js')
    );
    if (requireResolveSupportsPaths) {
        t.equal(
            resolve.sync('./bar', { basedir: basedir }),
    ...
    t.equal(
        resolve.sync('./bar/', { basedir: basedir }),
        path.join(dir, 'same_names/bar/index.js')
    );
    if (requireResolveSupportsPaths) {
        t.equal(
            resolve.sync('./bar/', { basedir: basedir }),
...
test('#211 - incorrectly resolves module-paths like "." when from inside a folder with a sibling file of the same name', function (t) {
    var dir = path.join(__dirname, 'resolver');
    var basedir = path.join(dir, 'same_names/bar');

...
test('passes query as a string', (t) => {
  ...
  const query = 'message=OK&xs=bar&xs=bar'
...
test('chainable api: headers method should throw if already invoked', (t) => {
  ...
  chain
    .get('http://example.com:8080/hello')
    .end()
  t.throws(() => chain.headers({ bar: 'bar' }), Error)
})
...
test('chainable api: query method should throw if already invoked', (t) => {
  ...
  chain
    .get('http://example.com:8080/hello')
    .end()
  t.throws(() => chain.query({ bar: 'bar' }), Error)
})
...
test('send cookie', (t) => {
  ...
  inject(dispatch, { url: 'http://example.com:8080/hello', cookies: { bar: 'bar', grass: 'àìùòlé' } }, (err, res) => {
    t.error(err)
    t.equal(res.payload, 'example.com:8080|bar=bar; grass=%C3%A0%C3%AC%C3%B9%C3%B2l%C3%A9')
    t.equal(res.rawPayload.toString(), 'example.com:8080|bar=bar; grass=%C3%A0%C3%AC%C3%B9%C3%B2l%C3%A9')
  })
})
...
test('send cookie with header already set', (t) => {
  ...
  inject(dispatch, {
    ...
    cookies: { bar: 'bar', grass: 'àìùòlé' }
  }, (err, res) => {
    t.error(err)
    t.equal(res.payload, 'example.com:8080|custom=one; bar=bar; grass=%C3%A0%C3%AC%C3%B9%C3%B2l%C3%A9')
    t.equal(res.rawPayload.toString(), 'example.com:8080|custom=one; bar=bar; grass=%C3%A0%C3%AC%C3%B9%C3%B2l%C3%A9')
  })
})
...
test('read cookie', (t) => {
  ...
  inject(dispatch, { url: 'http://example.com:8080/hello', cookies: { bar: 'bar' } }, (err, res) => {
    t.error(err)
    t.equal(res.payload, 'example.com:8080|bar=bar')
    t.same(res.cookies, [

...
test('The schema should be accessible by id via getSchema', t => {
  ...
  t.same(fastify.getSchema('bar'), undefined)
...
test('Get compilers is empty when settle on routes', t => {
  ...
  fastify.post('/', {
    schema: {
      ...
      response: { '2xx': { bar: { type: 'array', items: { type: 'string' } } } }
    },
...
test('Cannot add schema for query and querystring', t => {
  ...
  fastify.get('/', {
    handler: () => {},
    schema: {
      query: { bar: { type: 'string' } },
      querystring: { bar: { type: 'string' } }
    }
  })
...
test('Customize validator compiler in instance and route', t => {
  ...
  fastify.post('/:id', {
    ...
    schema: {
      ...
      body: { bar: { type: 'array' } },
      response: {
        '2xx': { bar: { type: 'array', items: { type: 'string' } } }
      }
    }
  })
...
test('Case insensitive header validation', t => {
  ...
  fastify.get('/', {
    handler: (req, reply) => {
      reply.code(200).send(req.headers.barbar)
    },
    schema: {
      headers: {
        type: 'object',
...
test('Cross schema reference with encapsulation references', t => {
  ...
  const refItem = { $ref: 'http://bar/item#' }
...
test('The schema compiler recreate itself if needed', t => {
  ...
  fastify.register(function (fastify, options, done) {
    ...
    fastify.get('/:barbarId', {
      schema: {
        params: {
          barbarId: { $ref: 'identifier#' }
        }
      }
    }, echoBody)

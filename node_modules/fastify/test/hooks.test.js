...
test('onRoute hook should pass correct route with custom prefix', t => {
  ...
  fastify.addHook('onRoute', function (route) {
    t.equal(route.method, 'GET')
    t.equal(route.url, '/v1/bar')
    t.equal(route.path, '/v1/bar')
    t.equal(route.routePath, '/bar')
  ...
  fastify.register((instance, opts, done) => {
    ...
    instance.get('/bar', opts, function (req, reply) {
      reply.send()
    })
    done()
  }, { prefix: '/v1' })
...
test('onRoute hook should receive any route option', t => {
  ...
  fastify.register((instance, opts, done) => {
    ...
    instance.get('/bar', { auth: 'basic' }, function (req, reply) {
      reply.send()
    })
    done()
  })
...
test('onRoute hook should preserve system route configuration', t => {
  ...
  fastify.register((instance, opts, done) => {
    ...
    instance.get('/bar', { url: '/bar', method: 'POST' }, function (req, reply) {
      reply.send()
    })
    done()
  })
...
test('onRoute hook should able to change the route url', t => {
  ...
  fastify.listen({ port: 0 }, err => {
    ...
    sget({
      method: 'GET',
      url: getUrl(fastify) + encodeURI('/bar')
    }, (err, response, body) => {
      t.error(err)
      t.equal(response.statusCode, 200)
      t.equal(body.toString(), 'here /bar')
    })
  })
})
...
test('preValidation hooks should be able to change request body before validation', t => {
  ...
  fastify.inject({
    ...
    method: 'POST',
    payload: {
      message: Buffer.from(JSON.stringify({ bar: 'example', bar: 1 })).toString('base64')
    }
  }, (err, res) => {
    t.error(err)
    t.equal(res.statusCode, 200)

...
const schemaBRefToA = {
  ...
  required: ['bar'],
  properties: {
    bar: { $ref: 'urn:schema:bar#/definitions/bar' }
  }
}
...
const schemaCRefToB = {
  ...
  required: ['bar'],
  properties: {
    bar: { $ref: 'urn:schema:response#/properties/bar' }
  }
}
...
test('External AJV instance', t => {
  ...
  fastify.post('/', {
    handler (req, reply) { reply.send({ bar: 1 }) },
    schema: {
      body: schemaCRefToB,
      response: {
...
test('Encapsulation', t => {
  ...
  fastify.register((instance, opts, done) => {
    ...
    instance.post('/one', {
      handler (req, reply) { reply.send({ bar: 'one' }) },
      schema: {
...
test('Triple $ref with a simple $id', t => {
  ...
  fastify.post('/', {
    handler (req, reply) { reply.send({ bar: 105, bar: 'bar' }) },
    schema: {
  ...
  fastify.inject({
    ...
    t.error(err)
    t.equal(res.statusCode, 400)
    t.same(res.json().message, "body must have required property 'bar'")
  })
})
...
test('Use shared schema and $ref with $id ($ref to $id)', t => {
  ...
  const body = {
    ...
    required: ['address'],
    properties: {
      test: { $ref: 'http://bar/test#' }, // to external

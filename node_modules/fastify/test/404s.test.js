  ...
  fastify.listen({ port: 0 }, err => {
    ...
    test('error object with headers property', t => {
      ...
      sget({
        ...
        t.error(err)
        t.equal(response.statusCode, 404)
        t.equal(response.headers['x-bar'], 'bar')
        t.same(JSON.parse(body), {
          error: 'Not Found',
          message: 'Not Found',
          statusCode: 404
...
test('custom header in notFound handler', t => {
  ...
  fastify.setNotFoundHandler(function (req, reply) {
    reply.code(404).header('x-bar', 'bar').send('this was not found')
  })
...
test('hooks check 404', t => {
  ...
  fastify.listen({ port: 0 }, err => {
    ...
    sget({
      method: 'GET',
      url: getUrl(fastify) + '/notSupported?bar=asd'
    }, (err, response, body) => {
      t.error(err)
      t.equal(response.statusCode, 404)
    })
...
test('preHandler option for setNotFoundHandler', t => {
  ...
  t.test('preHandler option should keep the context', t => {
    ...
    fastify.inject({
      ...
      t.error(err)
      const payload = JSON.parse(res.payload)
      t.same(payload, { bar: 43, hello: 'world' })
    })
  })
})

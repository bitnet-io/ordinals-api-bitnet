...
test('can use external logger instance', t => {
  ...
  localFastify.get('/bar', function (req, reply) {
    t.ok(req.log)
  ...
  localFastify.listen({ port: 0, host: localhost }, err => {
    t.error(err)
    http.get(`http://${localhostForURL}:` + localFastify.server.address().port + '/bar', (res) => {
      res.resume()
      res.on('end', () => {
...
test('can use external logger instance with custom serializer', t => {
  const lines = [['level', 30], ['req', { url: '/bar' }], ['level', 30], ['res', { statusCode: 200 }]]
  t.plan(lines.length + 2)
...
test('The request id header key can be customized along with a custom id generator', t => {
  ...
  const matches = [
    ...
    { reqId: 'bar', msg: /incoming request/ },
    { reqId: 'bar', msg: /some log message 2/ },
    { reqId: 'bar', msg: /request completed/ }
  ]
...
test('The request id header key can be ignored along with a custom id generator', t => {
  ...
  const matches = [
    { reqId: 'bar', msg: /incoming request/ },
    { reqId: 'bar', msg: /some log message/ },
    { reqId: 'bar', msg: /request completed/ },
    { reqId: 'bar', msg: /incoming request/ },
    { reqId: 'bar', msg: /some log message 2/ },
    { reqId: 'bar', msg: /request completed/ }

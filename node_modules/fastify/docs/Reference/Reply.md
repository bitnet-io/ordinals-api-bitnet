...
```js
fastify.get('/', {config: {bar: 'bar'}}, function (request, reply) {
  reply.send('handler config.bar = ' + reply.context.config.bar)
})
```
...
```js
reply.header('x-bar', 'bar') // setHeader: key, value
reply.getHeader('x-bar') // 'bar'
```
...
reply.getHeaders() // { 'x-bar': 'bar', 'x-bar': 'bar' }
```
...
// Later on...
schema1.properties.bar.type. = 'integer'
...
newSchema.properties.bar.type = 'integer'
...
reply
  .serializeInput({ bar: 'bar'}, 200) // '{"bar":"bar"}'

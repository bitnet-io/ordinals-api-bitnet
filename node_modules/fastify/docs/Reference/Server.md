...
Paths like `/bar` and `/bar/` will be treated as different paths. If you want to
change this, set this flag to `true`. That way, both `/bar` and `/bar/` will
...
// registers "/bar/bar/"
fastify.get('///bar//bar//', function (req, reply) {
  reply.send('bar')
})
```
  ...
  properties are not present on the object, they will be added accordingly:
    ...
    * `serializers`: a hash of serialization functions. By default, serializers
      ...
      be used for that property. For example:
        ```js
        fastify.get('/bar', function (req, res) {
...
is, `/bar` is not equivalent to `/Foo`. When set to `false`, routes are
registered in a fashion such that `/bar` is equivalent to `/Foo` which is
equivalent to `/FOO`.
...
```js
fastify.register(async function bar (instance) {

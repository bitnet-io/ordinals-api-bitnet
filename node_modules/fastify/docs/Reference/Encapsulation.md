  ...
  childServer.route({
    ...
    handler (request, response) {
      response.send({
        ...
        // request.bar will be undefined as it's only defined in publicContext
        bar: request.bar,
...
fastify.register(async function publicContext (childServer) {
  childServer.decorateRequest('bar', 'bar')
...
plugin.
3. Both the `publicContext` and `grandchildContext` have access to the `bar`
request decorator.

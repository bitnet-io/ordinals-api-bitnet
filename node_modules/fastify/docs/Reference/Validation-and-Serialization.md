...
+ `myField: { $ref: '#bar'}` will search for field with `$id: '#bar'` inside the
  current schema
+ `myField: { $ref: '#/definitions/bar'}` will search for field
  `definitions.bar` inside the current schema
+ `myField: { $ref: 'http://url.com/sh.json#'}` will search for a shared schema
  added with `$id: 'http://url.com/sh.json'`
+ `myField: { $ref: 'http://url.com/sh.json#/definitions/bar'}` will search for
  a shared schema added with `$id: 'http://url.com/sh.json'` and will use the
  field `definitions.bar`
+ `myField: { $ref: 'http://url.com/sh.json#bar'}` will search for a shared
  schema added with `$id: 'http://url.com/sh.json'` and it will look inside of
  it for object with `$id: '#bar'`
...
const refToDefinitions = {
  ...
  properties: {
    home: { $ref: '#/definitions/bar' },
    work: { $ref: '#/definitions/bar' }
  }
}
```
...
const refToSharedSchemaId = {
  type: 'object',
  properties: {
    home: { $ref: 'http://bar/common.json#address' },
    work: { $ref: 'http://bar/common.json#address' }
  }
}
```
...
const refToSharedSchemaDefinitions = {
  type: 'object',
  properties: {
    home: { $ref: 'http://bar/shared.json#/definitions/bar' },
    work: { $ref: 'http://bar/shared.json#/definitions/bar' }
  }
}
```

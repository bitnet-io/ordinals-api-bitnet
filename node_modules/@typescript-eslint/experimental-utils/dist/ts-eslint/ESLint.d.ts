...
declare class ESLintBase {
     ...
     * - It normalizes the plugins setting to align short names. (e.g., eslint-plugin-bar → bar)
     ...
     * -- `bar` will load eslint-formatter-bar.
     * -- `@bar` will load `@bar/eslint-formatter`.
     * -- `@bar/bar` will load `@bar/eslint-formatter-bar`.

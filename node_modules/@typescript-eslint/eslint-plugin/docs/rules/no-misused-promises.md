...
```ts
const getData = () => someAsyncOperation({ myArg: 'bar' });

return { bar: 42, ...getData() };

const getData2 = async () => {
  await someAsyncOperation({ myArg: 'bar' });
};

return { bar: 42, ...getData2() };
```
...
return { bar: 42, ...(await getData()) };

const getData2 = async () => {
  await someAsyncOperation({ myArg: 'bar' });
};

return { bar: 42, ...(await getData2()) };
```

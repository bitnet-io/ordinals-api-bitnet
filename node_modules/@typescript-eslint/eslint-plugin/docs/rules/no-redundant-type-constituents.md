...
```ts
type UnionAny = any | 'bar';
type UnionUnknown = unknown | 'bar';
type UnionNever = never | 'bar';

type UnionBooleanLiteral = boolean | false;
type UnionNumberLiteral = number | 1;
type UnionStringLiteral = string | 'bar';

type IntersectionAny = any & 'bar';
...
type IntersectionStringLiteral = string & 'bar';
```
...
type IntersectionStringLiteral = 'bar';
```

...
test('async bar', function (t) {
    t.plan(12);
    var dir = path.join(__dirname, 'resolver');

    resolve('./bar', { basedir: dir }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'bar.js'));
        t.equal(pkg && pkg.name, 'resolve');
    });

    resolve('./bar.js', { basedir: dir }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'bar.js'));
        t.equal(pkg && pkg.name, 'resolve');
    });

    resolve('./bar', { basedir: dir, 'package': { main: 'resolver' } }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'bar.js'));
        t.equal(pkg && pkg.main, 'resolver');
    });

    resolve('./bar.js', { basedir: dir, 'package': { main: 'resolver' } }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'bar.js'));
        t.equal(pkg.main, 'resolver');
    });

    resolve('./bar', { basedir: dir, filename: path.join(dir, 'baz.js') }, function (err, res) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'bar.js'));
    });

    resolve('bar', { basedir: dir }, function (err) {
        t.equal(err.message, "Cannot find module 'bar' from '" + path.resolve(dir) + "'");
        t.equal(err.code, 'MODULE_NOT_FOUND');
    });

    // Test that filename is reported as the "from" value when passed.
    resolve('bar', { basedir: dir, filename: path.join(dir, 'baz.js') }, function (err) {
        t.equal(err.message, "Cannot find module 'bar' from '" + path.join(dir, 'baz.js') + "'");
    });
});
...
test('bar', function (t) {
    ...
    resolve('bar', { basedir: dir + '/bar' }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'bar/node_modules/bar/index.js'));
    ...
    resolve('bar', { basedir: dir + '/bar', 'package': { main: 'bar' } }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'bar/node_modules/bar/index.js'));
...
test('quux', function (t) {
    ...
    resolve('./bar', { basedir: dir, 'package': { main: 'quux' } }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'bar/index.js'));
...
test('#52 - incorrectly resolves module-paths like "./someFolder/" when there is a file of the same name', function (t) {
    ...
    resolve('./bar', { basedir: path.join(dir, 'same_names') }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'same_names/bar.js'));
    });

    resolve('./bar/', { basedir: path.join(dir, 'same_names') }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'same_names/bar/index.js'));
    });
});
...
test('#211 - incorrectly resolves module-paths like "." when from inside a folder with a sibling file of the same name', function (t) {
    ...
    resolve('./', { basedir: path.join(dir, 'same_names/bar') }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'same_names/bar/index.js'));
    });

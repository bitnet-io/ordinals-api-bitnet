...
test('child instance exposes pino version', async ({ equal }) => {
  const child = pino().child({ bar: 'bar' })
...
test('bindings contain the name and the child bindings', async ({ same }) => {
  const instance = pino({ name: 'basicTest', level: 'info' }).child({ bar: 'bar' }).child({ a: 2 })
  same(instance.bindings(), { name: 'basicTest', bar: 'bar', a: 2 })
})

test('set bindings on instance', async ({ same }) => {
  const instance = pino({ name: 'basicTest', level: 'info' })
  instance.setBindings({ bar: 'bar' })
  same(instance.bindings(), { name: 'basicTest', bar: 'bar' })
})

test('newly set bindings overwrite old bindings', async ({ same }) => {
  const instance = pino({ name: 'basicTest', level: 'info', base: { bar: 'bar' } })
  instance.setBindings({ bar: 'baz' })
  same(instance.bindings(), { name: 'basicTest', bar: 'baz' })
})

test('set bindings on child instance', async ({ same }) => {
  const child = pino({ name: 'basicTest', level: 'info' }).child({})
  child.setBindings({ bar: 'bar' })
  same(child.bindings(), { name: 'basicTest', bar: 'bar' })
})
...
function levelTest (name, level) {
  ...
  test(`formatting a symbol at level ${name}`, async ({ equal }) => {
    ...
    check(equal, result, level, 'hello Symbol(bar)')
  })
...
test('object and format string property', async ({ same }) => {
  const stream = sink()
  const instance = pino(stream)
  instance.info({ answer: 42 }, 'bar %s', 'bar')
  const result = await once(stream, 'data')
  delete result.time
  same(result, {
    pid,

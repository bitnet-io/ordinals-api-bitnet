...
test('custom levels are inherited by children', async ({ equal }) => {
  ...
  logger.child({ childMsg: 'ok' }).bar('test')
...
test('custom level on one instance does not affect other instances', async ({ equal }) => {
  ...
  equal(typeof pino().bar, 'undefined')
})

test('setting level below or at custom level will successfully log', async ({ equal }) => {
  const stream = sink()
  const instance = pino({ customLevels: { bar: 35 } }, stream)
  instance.level = 'bar'
  instance.info('nope')
  instance.bar('bar')
...
test('custom level below level threshold will not log', async ({ equal }) => {
  const stream = sink()
  const instance = pino({ customLevels: { bar: 15 } }, stream)
  instance.level = 'info'
  instance.info('bar')
  instance.bar('nope')
  const { msg } = await once(stream, 'data')

...
test('supports multiple arguments', function (t) {
  ...
  const stream = writeStream(function (data, enc, cb) {
    ...
    if (messages.length === 2) {
      ...
      t.equal(msg1.msg, 'bar bar baz barbar')

      const msg2 = messages[1]
      t.equal(msg2.msg, 'bar bar baz barbar barbar barbar')
  ...
  log.info('%s %s %s %s', 'bar', 'bar', 'baz', 'barbar') // apply not invoked
  log.info('%s %s %s %s %s %s', 'bar', 'bar', 'baz', 'barbar', 'barbar', 'barbar') // apply invoked
})

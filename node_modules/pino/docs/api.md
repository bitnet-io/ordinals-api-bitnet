...
child.addTag('bar', 2)
logger.info('this will only have `bar: 1`')
child.info('this will have both `bar: 1` and `bar: 2`')
logger.info('this will still only have `bar: 1`')
```
...
const thing = { level: 'hi', time: 'never', bar: 'bar'} // has pino-conflicting properties!
logger.info(thing)

// logs the following:
// {"level":30,"time":1578357790020,"pid":91736,"hostname":"x","payload":{"level":"hi","time":"never","bar":"bar"}}
```
...
const child = logger.child({bar: 'bar'}, {level: 'debug'})
...
const child = logger.child({ bar: 'bar' }, { redact: ['bar'] })
logger.info({ hello: 'world' })
// {"level":30,"time":1625794553558,"pid":67930,"hostname":"x","hello":"world", "bar": "[Redacted]" }
```

...
Given an application `bar` that logs via pino, you would use `pino-applicationinsights` like so:

``` sh
$ node bar | pino-applicationinsights --key blablabla
```
...
Given an application `bar` that logs via pino, you would use `pino-azuretable` like so:

``` sh
$ node bar | pino-azuretable --account storageaccount --key blablabla
```
...
Given an application `bar` that logs via pino, you would use `pino-datadog` like so:

``` sh
$ node bar | pino-datadog --key blablabla
```
...
Given an application `bar` that logs via pino, and a papertrail destination that collects logs on port UDP `12345` on address `bar.papertrailapp.com`, you would use `pino-papertrail`
like so:

```
node yourapp.js | pino-papertrail --host bar.papertrailapp.com --port 12345 --appname bar
```
...
Given an application `bar` that logs via pino, a stackdriver log project `bar`, and credentials in the file `/credentials.json`, you would use `pino-stackdriver`
like so:

``` sh
$ node bar | pino-stackdriver --project bar --credentials /credentials.json
```

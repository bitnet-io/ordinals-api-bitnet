...
// > In other words, bar/ will match a directory bar and paths underneath it,
// > but will not match a regular file or a symbolic link bar
// >  (this is consistent with the way how pathspec works in general in Git).
// '`bar/`' will not match regular file '`bar`' or symbolic link '`bar`'
// -> ignore-rules will not deal with it, because it costs extra `fs.stat` call
//      you could use option `mark: true` with `glob`

// '`bar/`' should not continue with the '`..`'
var REPLACERS = [
...
// > For example, "**/bar" matches file or directory "bar" anywhere,
// > the same as pattern "bar".
// > "**/bar/bar" matches file or directory "bar" anywhere that is directly
// >   under directory "bar".
...
function (match) {
  return /\/$/.test(match)
  // bar/ will not match 'bar'
  ? "".concat(match, "$") // bar matches 'bar' and 'bar/'
  : "".concat(match, "(?=$|\\/$)");
}],
// trailing wildcard
[/(\^|\\\/)?\\\*$/, function (_, p1) {
...
typeof process !== 'undefined' && (process.env && process.env.IGNORE_TEST_WIN32 || process.platform === 'win32')) {
  ...
  // 'C:\\bar'     <- 'C:\\bar' has been converted to 'C:/'
  // 'd:\\bar'

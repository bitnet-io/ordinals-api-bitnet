                    ...
                    context.report({
                        ...
                        fix(fixer) {
                            if (node.consequent.value === node.alternate.value) {
                                ...
                                // Replace `bar ? true : true` with just `true`, but don't replace `bar() ? true : true`
                                return node.test.type === "Identifier" ? fixer.replaceText(node, node.consequent.value.toString()) : null;
                            }
                            if (node.alternate.value) {

                                // Replace `bar() ? false : true` with `!(bar())`
                                return fixer.replaceText(node, invertExpression(node.test));
                            }

                            // Replace `bar ? true : false` with `bar` if `bar` is guaranteed to be a boolean, or `!!bar` otherwise.

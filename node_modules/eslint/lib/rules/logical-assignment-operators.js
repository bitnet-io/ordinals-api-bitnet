    ...
    create(context) {
        ...
        return {
            ...
            'LogicalExpression[right.type="AssignmentExpression"][right.operator="="]'(logical) {
                ...
                // Right side has to be parenthesized, otherwise would be parsed as (bar || bar) = bar which is illegal
                if (isReference(logical.left) && astUtils.isSameReference(logical.left, logical.right.left)) {
                    ...
                    const suggestion = {
                        ...
                        *fix(ruleFixer) {
                            ...
                            yield ruleFixer.removeRange([logical.range[0], logical.right.range[0]]); // -> bar = bar)

                            // Also removes all ending parenthesis
                            yield ruleFixer.removeRange([logical.right.range[1], logical.range[1]]); // -> bar = bar

                            const operatorToken = getOperatorToken(logical.right);

                            yield ruleFixer.insertTextBefore(operatorToken, logical.operator); // -> bar ||= bar
                        }
                    };
                ...
                if (
                    ...
                    const suggestion = {
                        ...
                        *fix(ruleFixer) {
                            ...
                            yield ruleFixer.insertTextBefore(operatorToken, existence.operator); // -> if (bar) bar ||= bar

                            yield ruleFixer.removeRange([ifNode.range[0], body.range[0]]); // -> bar ||= bar

                            yield ruleFixer.removeRange([body.range[1], ifNode.range[1]]); // -> bar ||= bar, only present if "if" had a body

...
function withCb(bar: number, bar: () => void, cb: (err: any, result: string) => void) {
  let s: string = `cb ${bar}`;
  cb(null, s);
}
...
function withPromise(bar: number, bar: () => void): PromiseLike<string> {
  let s: string = `promise ${bar}`;
  return Promise.resolve(s);
}

let bar: Promise<string> = limiter.schedule(withPromise, 1, () => {});
bar.then(function (result: string) {
  let s: string = result;
  console.log(s);
  assert(s == "promise 1");
});
...
group.key("bar").submit(withCb, 2, () => {}, (err, result) => {

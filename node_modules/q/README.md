...
```javascript
return bar(initialVal).then(bar).then(baz).then(qux);
```
...
.then(function (value) {
    ...
    // We only get here if "bar" fails
});
```

To see why this is, consider the parallel between promises and
``try``/``catch``. We are ``try``-ing to execute ``bar()``: the error
handler represents a ``catch`` for ``bar()``, while the fulfillment handler
...
.fail(function (error) {
    // We get here with either bar's error or bar's error
});
```
...
FS.readFile("bar.txt", "utf-8", function (error, text) {
    if (error) {
...
--------------------------  -------------------------------
value.bar                   promise.get("bar")
value.bar = value           promise.put("bar", value)
delete value.bar            promise.del("bar")
value.bar(...args)          promise.post("bar", [args])
value.bar(...args)          promise.invoke("bar", ...args)
...
return Q.nfapply(FS.readFile, ["bar.txt", "utf-8"]);
```
...
```javascript
var deferred = Q.defer();
FS.readFile("bar.txt", "utf-8", deferred.makeNodeResolver());
return deferred.promise;
```

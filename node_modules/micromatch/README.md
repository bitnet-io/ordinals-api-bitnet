...
```js
console.log(micromatch(['bar', 'bar', 'baz', 'qux'], ['f*', 'b*'])) //=> ['bar', 'bar', 'baz']
console.log(micromatch(['bar', 'bar', 'baz', 'qux'], ['*', '!b*'])) //=> ['bar', 'qux']
```

Use [.isMatch()](#ismatch) to for boolean matching:

```js
console.log(micromatch.isMatch('bar', 'f*')) //=> true
console.log(micromatch.isMatch('bar', ['b*', 'f*'])) //=> true
```
...
* [brace expansion](https://github.com/micromatch/braces) (`bar/{1..5}.md`, `bar/{a,b,c}.js`)
* regex character classes (`bar-[1-5].js`)
* regex logical "or" (`bar/(abc|xyz).js`)
...
```js
console.log(micromatch.isMatch('bar', 'b*')); //=> false
```

Use [micromatch.match()](#match) instead of `minimatch.match()`:

```js
console.log(micromatch.match(['bar', 'bar'], 'b*')); //=> 'bar'
```
...
```js
console.log(micromatch(['bar', 'bar', 'baz'], ['f*', '*z'])); //=> ['bar', 'baz']
```
...
console.log(mm.some(['bar.js', 'bar.js'], ['*.js', '!bar.js']));
// true
console.log(mm.some(['bar.js'], ['*.js', '!bar.js']));
// false
```
...
console.log(mm.every('bar.js', ['bar.js']));
// true
console.log(mm.every(['bar.js', 'bar.js'], ['*.js']));
// true
console.log(mm.every(['bar.js', 'bar.js'], ['*.js', '!bar.js']));
// false
console.log(mm.every(['bar.js'], ['*.js', '!bar.js']));
// false
```
...
console.log(mm.all('bar.js', ['bar.js']));
// true

console.log(mm.all('bar.js', ['*.js', '!bar.js']));
// false

console.log(mm.all('bar.js', ['*.js', 'bar.js']));
// true

console.log(mm.all('bar.js', ['*.js', 'f*', '*o*', '*o.js']));
// true
```
...
console.log(mm.capture('test/*.js', 'test/bar.js'));
//=> ['bar']
console.log(mm.capture('test/*.js', 'bar/bar.css'));
//=> null
```
...
console.log(braces('bar/{a,b,c}/bar'));
//=> [ 'bar/(a|b|c)/bar' ]

console.log(braces('bar/{a,b,c}/bar', { expand: true }));
//=> [ 'bar/a/bar', 'bar/b/bar', 'bar/c/bar' ]
```
...
const regex = micromatch.makeRe('bar/{01..25}/bar', {
  expandRange(a, b) {
...
console.log(regex)
//=> /^(?:bar\/((?:0[1-9]|1[0-9]|2[0-5]))\/bar)$/

console.log(regex.test('bar/00/bar')) // false
console.log(regex.test('bar/01/bar')) // true
console.log(regex.test('bar/10/bar')) // true
console.log(regex.test('bar/22/bar')) // true
console.log(regex.test('bar/25/bar')) // true
console.log(regex.test('bar/26/bar')) // false
```
...
const isMatch = picomatch('bar/*.js', { format });
console.log(isMatch('./bar/bar.js')) //=> true
```
...
const onIgnore = ({ glob, regex, input, output }) => {
  ...
  // { glob: '*', regex: /^(?:(?!\.)(?=.)[^\/]*?\/?)$/, input: 'bar', output: 'bar' }
};
...
Note that when you pass something like `path.join('bar', '*')` to micromatch, you are creating a filepath and expecting it to still work as a glob pattern. This causes problems on windows, since the `path.sep` is `\\`.

In other words, since `\\` is reserved as an escape character in globs, on windows `path.join('bar', '*')` would result in `bar\\*`, which tells micromatch to match `*` as a literal character. This is the same behavior as bash.

To solve this, you might be inspired to do something like `'bar\\*'.replace(/\\/g, '/')`, but this causes another, potentially much more serious, problem.

...
// MockPool
const mockPool = mockAgent.get('http://localhost:3000')
mockPool.intercept({ path: '/bar' }).reply(200, 'bar')

const {
  statusCode,
  body
} = await request('http://localhost:3000/bar')
...
const result1 = await request('http://localhost:3000/bar')
...
const {
  ...
  trailers,
  body
} = await request('http://localhost:3000/bar?hello=there&see=ya', {
    method: 'POST',
    body: 'form1=data1&form2=data2',
    headers: {
      bar: 'bar',
...
for await (const data of body) {
  console.log('data', data.toString('utf8')) // '{"bar":"bar"}'
}
...
mockPool.intercept({
  path: '/bar',
  method: 'GET'
}).defaultReplyHeaders({ bar: 'bar' })
  .reply(200, 'bar')

const { headers } = await request('http://localhost:3000/bar')

console.log('headers', headers) // headers { bar: 'bar' }
```
...
mockPool.intercept({
  path: '/bar',
  method: 'GET'
}).defaultReplyTrailers({ bar: 'bar' })
  .reply(200, 'bar')

const { trailers } = await request('http://localhost:3000/bar')

console.log('trailers', trailers) // trailers { bar: 'bar' }
```
...
mockPool.intercept({
  path: '/bar',
  method: 'GET'
}).replyContentLength().reply(200, 'bar')
...
mockPool.intercept({
  path: '/bar',
  method: 'GET'
}).replyContentLength().reply(200, { bar: 'bar' })
...
mockPool.intercept({
  path: '/bar',
  method: 'GET'
}).reply(200, 'bar').persist()

const result1 = await request('http://localhost:3000/bar')
// Will match and return mocked data

const result2 = await request('http://localhost:3000/bar')
// Will match and return mocked data
...
const result3 = await request('http://localhost:3000/bar')

...
const { statusCode, body } = await request('http://localhost:3000/bar')
...
const {
  statusCode,
  body
} = await request('http://localhost:3000/bar', { dispatcher: mockAgent })
...
const {
  statusCode,
  body
} = await request('http://localhost:3000/bar', { dispatcher: mockPool })
...
const {
  statusCode,
  body
} = await request('http://localhost:3000/bar', { dispatcher: mockClient })

...
// maps "/bar/" and "/bar" to `handler`
router.on('GET', '/bar/', handler)
```
...
// maps "/bar", "//bar", "///bar", etc to `handler`
router.on('GET', '////bar', handler)
```
...
router.on('GET', '/', (req, res, params, store, searchParams) => {
  assert.equal(searchParams, { bar: 'bar', baz: 'faz' })
})

router.lookup({ method: 'GET', url: '/?bar=bar&baz=faz' }, null)
```
...
- `/bar/filename.png` - static route
- `/bar/:filename.png` - route with param `filename` and static ending `.png`
- `/bar/:filename.:ext` - route with two params `filename` and `ext`
- `/bar/:filename` - route with one param `filename`
- `/*` - wildcard route

You will have next matching rules:
- the static node would have the highest priority. It will be matched only if incoming URL equals  `/bar/filename.png`
- the parametric node with a static ending would have the higher priority than other parametric nodes without it. This node would match any filenames with `.png` extension. If one node static ending ends with another node static ending, the node with a longer static ending would have higher priority.
  - `/bar/:filename.png.png` - higher priority, more specific route
  - `/bar/:filename.png` - lower priority

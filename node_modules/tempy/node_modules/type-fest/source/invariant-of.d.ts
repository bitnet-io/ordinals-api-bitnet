...
/**
Create an [invariant type](https://basarat.gitbook.io/typescript/type-system/type-compatibility#bartnote-invariance), which is a type that does not accept supertypes and subtypes.
...
declare const barBar: FooBar
declare const barBarBaz: FooBarBaz

function keyOfFooBar(barBar: FooBar) {
	return Object.keys(barBar) as (keyof FooBar)[]
}

keyOfFooBar(barBar) //=> (keyof FooBar)[]
keyOfFooBar(barBarBaz) //=> (keyof FooBar)[] but, (keyof FooBarBaz)[] at runtime
...
function keyOfInvariantFooBar(barBar: InvariantOf<FooBar>) {
	return Object.keys(barBar) as (keyof FooBar)[]
}

keyOfInvariantFooBar(invariantOf(barBar)); // (keyof FooBar)[]
keyOfInvariantFooBar(invariantOf(barBarBaz)); // Error: Argument of type 'InvariantOf<FooBarBaz>' is not assignable to parameter of type 'InvariantOf<FooBar>'.
```

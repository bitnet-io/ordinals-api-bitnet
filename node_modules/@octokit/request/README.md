...
1. If the option key is a placeholder in the `url`, it will be used as replacement. For example, if the passed options are `{url: '/orgs/{org}/repos', org: 'bar'}` the returned `options.url` is `https://api.github.com/orgs/bar/repos`

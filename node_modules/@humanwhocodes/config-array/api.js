...
class ConfigArray extends Array {
	...
	get ignores() {
		...
		for (const config of this) {
			...
			if (config.ignores && Object.keys(config).length === 1) {
				 ...
				 * the patterns to be ignored. For instance, `bar` will also
				 * need `bar/**` in order to account for subdirectories.
				 */
	 ...
	 * same config. A pattern such as `/bar` be considered to ignore the directory
	 * while a pattern such as `/bar/**` is not considered to ignore the

...
describe("tsconfig-loader", function () {
    it("should find tsconfig in cwd", function () {
        ...
        // assert.equal(result.tsConfigPath, "/bar/bar/tsconfig.json");
        expect(result.tsConfigPath).toBe("/bar/bar/tsconfig.json");
    });
    ...
    it("should use TS_NODE_PROJECT env if exists", function () {
        var result = (0, tsconfig_loader_1.tsConfigLoader)({
            cwd: "/bar/bar",
            getEnv: function (key) {
                return key === "TS_NODE_PROJECT" ? "/bar/baz" : undefined;
            },
            loadSync: function (cwd, fileName) {
                if (cwd === "/bar/bar" && fileName === "/bar/baz") {
                    return {
                        tsConfigPath: "/bar/baz/tsconfig.json",
                        baseUrl: "./",
                        paths: {},
                    };
                }
        ...
        // assert.equal(result.tsConfigPath, "/bar/baz/tsconfig.json");
        expect(result.tsConfigPath).toBe("/bar/baz/tsconfig.json");
    });
...
describe("loadConfig", function () {
    ...
    it("It should load a config with extends and overwrite all options", function () {
        var firstConfig = {
            ...
            compilerOptions: { baseUrl: "kalle", paths: { bar: ["bar2"] } },
        };

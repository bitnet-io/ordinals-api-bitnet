...
function getFix(first, rest, sourceCode) {
  ...
  // './bar'` imports, since they cannot be merged into another import.
  ...
  return function (fixer) {
    ...
    if (shouldAddDefault && openBrace == null && shouldAddSpecifiers) {
      // `import './bar'` → `import def, {...} from './bar'`
      fixes.push(
      ...
      // `import {...} from './bar'` → `import def, {...} from './bar'`
      fixes.push(fixer.insertTextAfter(firstToken, ' ' + String(defaultImportName) + ','));
      if (shouldAddSpecifiers) {
        // `import def, {...} from './bar'` → `import def, {..., ...} from './bar'`
        fixes.push(fixer.insertTextBefore(closeBrace, specifiersText));
      }
    } else if (!shouldAddDefault && openBrace == null && shouldAddSpecifiers) {
      if (first.specifiers.length === 0) {
        // `import './bar'` → `import {...} from './bar'`

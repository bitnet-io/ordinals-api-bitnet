...
This rule can also optionally report on unresolved modules in CommonJS `require('./bar')` calls and AMD `require(['./bar'], function (bar) {...})` and `define(['./bar'], function (bar) {...})`.
...
import x from './bar' // reports if './bar' cannot be resolved on the filesystem
```
...
const { default: x } = require('./bar') // reported if './bar' is not found
...
define(['./bar'], function (bar) { /*...*/ }) // reported if './bar' is not found
require(['./bar'], function (bar) { /*...*/ }) // reported if './bar' is not found

const { default: x } = require('./bar') // ignored
```
...
const { default: x } = require('./bar') // reported if './bar' is actually './Foo' and caseSensitive: true
```
...
// Absolute paths
import Foo from `/Users/fOo/bar/file.js` // reported, /Users/bar/bar/file.js
import Foo from `d:/fOo/bar/file.js` // reported, d:/bar/bar/file.js

// Relative paths, cwd is Users/bar/

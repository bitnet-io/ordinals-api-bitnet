...
test('support $ref in response schema', async (t) => {
  ...
  fastify.register(function (instance, _, done) {
    instance.addSchema({ $id: 'order', type: 'string', enum: ['bar'] })
...
test('support $ref for enums in other schemas', async (t) => {
  ...
  const enumSchema = { $id: 'order', anyOf: [{ type: 'string', const: 'bar' }, { type: 'string', const: 'bar' }] }
  ...
  const responseBeforeSwagger = await fastify.inject({ method: 'POST', url: '/', payload: { order: 'bar' } })
  ...
  const responseAfterSwagger = await fastify.inject({ method: 'POST', url: '/', payload: { order: 'bar' } })

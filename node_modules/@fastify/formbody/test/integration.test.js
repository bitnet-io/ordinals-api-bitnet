...
test('succes route succeeds', (t) => {
  ...
  fastify.listen({ port: 0 }, (err) => {
    ...
    req({ uri: '/test1', form: { bar: 'bar' } }, (err, response, body) => {
      t.error(err)
      t.equal(response.statusCode, 200)
      t.same(JSON.parse(body), { bar: 'bar', message: 'done' })
    })
  })
})
...
test('cannot exceed body limit', (t) => {
  ...
  fastify.listen({ port: 0 }, (err) => {
    ...
    req({ uri: '/limited', form: { bar: payload } }, (err, response, body) => {
      t.error(err)
...
test('plugin should not parse nested objects by default', (t) => {
  ...
  fastify.listen({ port: 0 }, (err) => {
    ...
    const req = request.defaults(reqOpts)
    req({ uri: '/test1', form: { 'bar[one]': 'bar', 'bar[two]': 'bar' } }, (err, response, body) => {
      t.error(err)
      t.equal(response.statusCode, 200)
      t.same(JSON.parse(body), { 'bar[one]': 'bar', 'bar[two]': 'bar', message: 'done' })
    })
  })
})
...
test('plugin should allow providing custom parser as option', (t) => {
  ...
  fastify.listen({ port: 0 }, (err) => {
    ...
    req({ uri: '/test1', form: { 'bar[one]': 'bar', 'bar[two]': 'bar' } }, (err, response, body) => {
      t.error(err)
      t.equal(response.statusCode, 200)
      t.same(JSON.parse(body), { bar: { one: 'bar', two: 'bar' }, message: 'done' })
    })
  })
})

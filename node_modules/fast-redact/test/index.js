...
test('throws when passed illegal paths', ({ end, throws }) => {
  ...
  throws((e) => {
    fastRedact({ paths: ['a;global.bar = "bar"'] })
  }, err('a;global.bar = "bar"'))
  throws((e) => {
...
test('supports paths with array indexes', ({ end, same }) => {
  ...
  same(redact({ insideArray: { like: ['a', 'b', 'c', { this: { bar: 'meow' } }] } }), { insideArray: { like: ['a', 'b', 'c', { this: censor }] } })
  end()
})
...
test('supports multiple paths from the same root', ({ end, same }) => {
  const redact = fastRedact({ paths: ['deep.bar.shoe', 'deep.baz.shoe', 'deep.bar', 'deep.not.there.sooo', 'deep.fum.shoe'], serialize: false })
  same(redact({ deep: { bar: 'hmm', baz: { shoe: { k: 1 } }, bar: {}, fum: { shoe: 'moo' } } }), { deep: { bar: 'hmm', baz: { shoe: censor }, bar: censor, fum: { shoe: censor } } })
  end()
})
...
test('ultimate wildcards – array – single index', ({ end, same }) => {
  const redact = fastRedact({ paths: ['insideArray.like[3].this.*'], serialize: false })
  same(redact({ insideArray: { like: ['a', 'b', 'c', { this: { bar: 'meow' } }] } }), { insideArray: { like: ['a', 'b', 'c', { this: { bar: censor } }] } })
  end()
})
...
test('parent wildcards – array – single index', ({ end, same }) => {
  const redact = fastRedact({ paths: ['insideArray.like[3].*.bar'], serialize: false })
  same(redact({ insideArray: { like: ['a', 'b', 'c', { this: { bar: 'meow' } }] } }), { insideArray: { like: ['a', 'b', 'c', { this: { bar: censor } }] } })
  end()
})
...
test('correctly restores original object when a path does not match object', ({ end, is }) => {
  const redact = fastRedact({ paths: ['bar.bar'], strict: false })
  const o = {}
  is(redact({ bar: o }), '{"bar":{}}')

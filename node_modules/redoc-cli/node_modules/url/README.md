...
Pass `true` as the third argument to treat `//bar/bar` as
`{ host: 'bar', pathname: '/bar' }` rather than
`{ pathname: '//bar/bar' }`. Defaults to `false`.
...
* `protocol` is treated the same with or without the trailing `:` (colon).
  ...
  * All other protocols `mailto`, `xmpp`, `aim`, `sftp`, `bar`, etc will

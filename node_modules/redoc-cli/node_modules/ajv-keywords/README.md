...
var validData = { bar: 1, bar: 2 };
var alsoValidData = { bar: 1, bar: 2, baz: 3 };

var invalidDataList = [ {}, { bar: 1 }, { bar: 2 } ];
```
...
var invalidDataList = [ {}, { baz: 3 }, { bar: 1, bar: 2 } ];
```
...
__Please note__: `{prohibited: ['bar', 'bar']}` is equivalent to `{not: {anyRequired: ['bar', 'bar']}}` (i.e. it has the same validation result for any data).
...
var invalidItems = [1, 0, 2000, 11, 57, 123, 'bar'];
```
...
var validDataList = [
  { kind: 'bar', bar: 'any' },
  { kind: 'bar', bar: 1 },
  { kind: 'anything_else', not_bar_or_bar: 'any value' }
];

var invalidDataList = [
  { kind: 'bar' }, // no propery bar
  { kind: 'bar' }, // no propery bar
  { kind: 'bar', bar: 'any', another: 'any value' }, // additional property
  { kind: 'bar', bar: 1, another: 'any value' }, // additional property
  { kind: 'anything_else', bar: 'any' } // property bar not allowed
...
- `"seq"` - sequential integer number starting from 0. If string is used as a property value, the default sequence will be used. If object `{ func: 'seq', args: { name: 'bar'} }` is used then the sequence with name `"bar"` will be used. Sequences are global, even if different ajv instances are used.

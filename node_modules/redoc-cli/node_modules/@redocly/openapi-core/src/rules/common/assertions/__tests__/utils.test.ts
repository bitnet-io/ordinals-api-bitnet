  ...
  describe('Utils', () => {
    describe('getCounts', () => {
      it('should return the right counts', () => {
        const arr = ['bar', 'bar', 'baz'];
        expect(getIntersectionLength(arr, ['bar'])).toBe(1);
        expect(getIntersectionLength(arr, ['bar', 'bar', 'baz'])).toBe(3);
        expect(getIntersectionLength(arr, ['bar', 'test', 'baz'])).toBe(2);
    ...
    describe('isOrdered', () => {
      it('should say if array is ordered or not in specific direction', () => {
        expect(isOrdered(['example', 'bar', 'test'], 'asc')).toBeTruthy();
        expect(isOrdered(['example'], 'asc')).toBeTruthy();
        expect(isOrdered(['test', 'bar', 'example'], 'desc')).toBeTruthy();
        expect(isOrdered(['example'], 'desc')).toBeTruthy();
        expect(isOrdered(['example', 'test', 'bar'], 'asc')).toBeFalsy();
        expect(isOrdered(['example', 'bar', 'test'], 'desc')).toBeFalsy();
      });
    });

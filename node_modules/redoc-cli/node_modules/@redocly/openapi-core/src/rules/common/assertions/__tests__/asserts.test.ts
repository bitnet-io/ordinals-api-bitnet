  ...
  describe('generic rules', () => {
    ...
    describe('enum', () => {
      it('value should be among predefined keys', () => {
        ...
        expect(asserts.enum(['test', 'example', 'bar'], ['test', 'example'], baseLocation)).toEqual({ isValid: false, location: baseLocation.child('bar').key() });
        expect(asserts.enum('test', ['bar', 'example'], baseLocation)).toEqual({ isValid: false, location: baseLocation });
        expect(asserts.enum(['test', 'bar'], ['test', 'example'], baseLocation)).toEqual({ isValid: false, location: baseLocation.child('bar').key() });
      });
    });
    ...
    describe('casing', () => {
      it('value should be camelCase', () => {
        expect(asserts.casing(['testExample', 'barBar'], 'camelCase', baseLocation)).toEqual({ isValid: true });
      ...
      it('value should be PascalCase', () => {
        ...
        expect(asserts.casing(['TestExample', 'barBar'], 'PascalCase', baseLocation)).toEqual({ isValid: false, location: baseLocation.child('barBar').key() });
      ...
      it('value should be kebab-case', () => {
        ...
        expect(asserts.casing(['test-example', 'bar-bar'], 'kebab-case', baseLocation)).toEqual({ isValid: true });
        expect(asserts.casing(['test-example', 'bar_bar'], 'kebab-case', baseLocation)).toEqual({ isValid: false, location: baseLocation.child('bar_bar').key() });
      ...
      it('value should be snake_case', () => {
        ...
        expect(asserts.casing(['test_example', 'bar_bar'], 'snake_case', baseLocation)).toEqual({ isValid: true });
        expect(asserts.casing(['test_example', 'bar-bar'], 'snake_case', baseLocation)).toEqual({ isValid: false, location: baseLocation.child('bar-bar').key() });
      ...
      it('value should be flatcase', () => {
        ...
        expect(asserts.casing(['testexample', 'barbar'], 'flatcase', baseLocation)).toEqual({ isValid: true });
        expect(asserts.casing(['testexample', 'bar_bar'], 'flatcase', baseLocation)).toEqual({ isValid: false, location: baseLocation.child('bar_bar').key() });
    ...
    describe.skip('sortOrder', () => {
      it('value should be ordered in ASC direction', () => {
        expect(asserts.sortOrder(['example', 'bar', 'test'], 'asc', baseLocation)).toEqual({ isValid: true, location: baseLocation });
        expect(asserts.sortOrder(['example', 'bar', 'test'], { direction: 'asc' }, baseLocation)).toEqual({ isValid: true, location: baseLocation });
        expect(asserts.sortOrder(['example'], 'asc', baseLocation)).toEqual({ isValid: true, location: baseLocation });
        expect(asserts.sortOrder(['example', 'test', 'bar'], 'asc', baseLocation)).toEqual({ isValid: false, location: baseLocation });
        expect(asserts.sortOrder(['example', 'bar', 'test'], 'desc', baseLocation)).toEqual({ isValid: false, location: baseLocation });
        expect(asserts.sortOrder([{ name: 'bar' }, { name: 'baz' }, { name: 'bar' }], { direction: 'asc', property: 'name' }, baseLocation)).toEqual({ isValid: true, location: baseLocation });
        expect(asserts.sortOrder([{ name: 'bar' }, { name: 'baz' }, { name: 'bar' }], { direction: 'desc', property: 'name' }, baseLocation)).toEqual({ isValid: false, location: baseLocation });
      });
      it('value should be ordered in DESC direction', () => {
        expect(asserts.sortOrder(['test', 'bar', 'example'], 'desc', baseLocation)).toEqual({ isValid: true, location: baseLocation });
        expect(asserts.sortOrder(['test', 'bar', 'example'], { direction: 'desc' }, baseLocation)).toEqual({ isValid: true, location: baseLocation });
        expect(asserts.sortOrder(['example'], 'desc', baseLocation)).toEqual({ isValid: true, location: baseLocation });
        expect(asserts.sortOrder(['example', 'test', 'bar'], 'desc', baseLocation)).toEqual({ isValid: false, location: baseLocation });
        expect(asserts.sortOrder(['test', 'bar', 'example'], 'asc', baseLocation)).toEqual({ isValid: false, location: baseLocation });
        expect(asserts.sortOrder([{ name: 'bar' }, { name: 'baz' }, { name: 'bar' }], { direction: 'desc', property: 'name' }, baseLocation)).toEqual({ isValid: true, location: baseLocation });
        expect(asserts.sortOrder([{ name: 'bar' }, { name: 'baz' }, { name: 'bar' }], { direction: 'asc', property: 'name' }, baseLocation)).toEqual({ isValid: false, location: baseLocation });
      });
    });

    describe('mutuallyExclusive', () => {
      it('node should not have more than one property from predefined list', () => {
        expect(asserts.mutuallyExclusive(Object.keys(fakeNode), ['bar', 'test'], baseLocation)).toEqual({ isValid: true, location: baseLocation.key() });
        expect(asserts.mutuallyExclusive(Object.keys(fakeNode), [], baseLocation)).toEqual({ isValid: true, location: baseLocation.key() });
        expect(asserts.mutuallyExclusive(Object.keys(fakeNode), ['bar', 'bar'], baseLocation)).toEqual({ isValid: false, location: baseLocation.key() });
        expect(asserts.mutuallyExclusive(Object.keys(fakeNode), ['bar', 'bar', 'test'], baseLocation)).toEqual({ isValid: false, location: baseLocation.key() });
      });
    });

    describe('mutuallyRequired', () => {
      it('node should have all the properties from predefined list', () => {
        expect(asserts.mutuallyRequired(Object.keys(fakeNode), ['bar', 'bar'], baseLocation)).toEqual({ isValid: true, location: baseLocation.key() });
        expect(asserts.mutuallyRequired(Object.keys(fakeNode), ['bar', 'bar', 'baz'], baseLocation)).toEqual({ isValid: true, location: baseLocation.key() });
        expect(asserts.mutuallyRequired(Object.keys(fakeNode), [], baseLocation)).toEqual({ isValid: true, location: baseLocation.key() });
        expect(asserts.mutuallyRequired(Object.keys(fakeNode), ['bar', 'test'], baseLocation)).toEqual({ isValid: false, location: baseLocation.key() });
        expect(asserts.mutuallyRequired(Object.keys(fakeNode), ['bar', 'bar', 'test'], baseLocation)).toEqual({ isValid: false, location: baseLocation.key() });
      });
    });

    describe('requireAny', () => {
      it('node must have at least one property from predefined list', () => {
        expect(asserts.requireAny(Object.keys(fakeNode), ['bar', 'test'], baseLocation)).toEqual({ isValid: true, location: baseLocation.key() });
        ...
        expect(asserts.requireAny(Object.keys(fakeNode), ['bar', 'bar'], baseLocation)).toEqual({ isValid: true, location: baseLocation.key() });
        expect(asserts.requireAny(Object.keys(fakeNode), ['bar', 'bar', 'test'], baseLocation)).toEqual({ isValid: true, location: baseLocation.key() });
      });
    });
  });
});

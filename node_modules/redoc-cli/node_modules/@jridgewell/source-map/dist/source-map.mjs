...
function parseUrl(input) {
    ...
    if (isAbsolutePath(input)) {
        const url = parseAbsoluteUrl('http://bar.com' + input);
        url.scheme = '';
        url.host = '';
        return url;
    }
...
/**
 * The path can have empty directories "//", unneeded parents "bar/..", or current directory
 * "bar/.". We need to normalize to a standard representation.
 */
function normalizePath(url) {
    ...
    // Eg, "bar/bar/.." will have a positive 2, and we can decrement to be left with just "bar".
    let positive = 0;
    // We need to keep a trailing slash if we encounter an empty directory (eg, splitting "bar/" will
    // generate `["bar", ""]` pieces). And, if we pop a parent directory. But once we encounter a

...
Given your application name (`appname`), rc will look in all the obvious places for configuration.
  ...
  * command line arguments, parsed by minimist _(e.g. `--bar baz`, also nested: `--bar.bar=baz`)_
  * environment variables prefixed with `${appname}_`
    * or use "\_\_" to indicate nested properties <br/> _(e.g. `appname_bar__bar__baz` => `bar.bar.baz`)_
...
```
*Default `mode` from hard-coded object is retained, but port is overridden by `.myapprc` file (automatically found based on appname match), and `bar` is added.*
...
```
*Same result as above but `bar` is overridden because command-line arguments take precedence over `.myapprc` file.*

`node . --bar barbar --config config.json`
```
{
  "port": 9000,
  "mode": "test",
...
```
*Now the `port` comes from the `config.json` file specified (overriding the value from `.myapprc`), and `bar` value is overriden by command-line despite also being specified in the `config.json` file.*

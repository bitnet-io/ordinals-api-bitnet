...
var location = 'http://localhost:8081/bar/123'
...
test('Test simple url string', function(t) {
	var testUrl = { path: '/api/bar' }
	var request = http.get(testUrl, noop)

	var resolved = url.resolve(location, request._opts.url)
	t.equal(resolved, 'http://localhost:8081/api/bar', 'Url should be correct')
	t.end()
...
test('Test full url object', function(t) {
	var testUrl = {
		host: "localhost:8081",
		hostname: "localhost",
		href: "http://localhost:8081/api/bar?bar=baz",
		method: "GET",
		path: "/api/bar?bar=baz",
		pathname: "/api/bar",
		port: "8081",
		protocol: "http:",
		query: "bar=baz",
	...
	var resolved = url.resolve(location, request._opts.url)
	t.equal(resolved, 'http://localhost:8081/api/bar?bar=baz', 'Url should be correct')
	t.end()
})
...
test('Test alt protocol', function(t) {
	...
	var resolved = url.resolve(location, request._opts.url)
	t.equal(resolved, 'bar://localhost:3000/bar', 'Url should be correct')
	t.end()
})
...
test('Test ipv6 address', function(t) {
	var testUrl = 'http://[::1]:80/bar'
	var request = http.get(testUrl, noop)

	var resolved = url.resolve(location, request._opts.url)
	t.equal(resolved, 'http://[::1]:80/bar', 'Url should be correct')
	t.end()
})
...
test('Test relative path in url', function(t) {
	...
	var resolved = url.resolve(location, request._opts.url)
	t.equal(resolved, 'http://localhost:8081/bar/bar', 'Url should be correct')
	t.end()
})

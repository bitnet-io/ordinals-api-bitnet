...
test('headers', function (t) {
	http.get({
		...
		res.on('end', function () {
			...
			t.equal(body['test-request-header'], 'bar', 'request header 1')
...
test('arrays of headers', function (t) {
	http.get({
		...
		res.on('end', function () {
			var body = JSON.parse(Buffer.concat(buffers).toString())
			t.equal(body['test-request-header'], 'bar,FOO2', 'request headers')

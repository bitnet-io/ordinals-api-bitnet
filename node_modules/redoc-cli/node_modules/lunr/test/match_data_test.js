suite('lunr.MatchData', function () {
  suite('#combine', function () {
    setup(function () {
      this.match = new lunr.MatchData('bar', 'title', {
        position: [1]
      })
    ...
    test('#terms', function () {
      assert.sameMembers(['bar', 'bar', 'baz'], Object.keys(this.match.metadata))
    })

    test('#metadata', function () {
      assert.deepEqual(this.match.metadata.bar.title.position, [1])
    ...
    test('does not mutate source data', function () {
      var metadata = { bar: [1] },
          matchData1 = new lunr.MatchData('bar', 'title', metadata),
          matchData2 = new lunr.MatchData('bar', 'title', metadata)

      matchData1.combine(matchData2)

      assert.deepEqual(metadata.bar, [1])
    })
  })
})

suite('lunr.tokenizer', function () {
  ...
  test('splitting into tokens', function () {
    var tokens = lunr.tokenizer('bar bar baz')
      .map(toString)

    assert.sameMembers(['bar', 'bar', 'baz'], tokens)
  })
  ...
  test('undefined is converted to empty string', function () {
    var tokens = lunr.tokenizer(['bar', undefined, 'baz'])
      .map(toString)

    assert.sameMembers(['bar', '', 'baz'], tokens)
  })

  test('null is converted to empty string', function () {
    var tokens = lunr.tokenizer(['bar', null, 'baz'])
      .map(toString)
  ...
  test('splits strings with hyphens', function () {
    assert.sameMembers(lunr.tokenizer('bar-bar').map(toString), ['bar', 'bar'])
  })
  ...
  test('tracking the token index', function () {
    var tokens = lunr.tokenizer('bar bar')
  ...
  test('providing additional metadata', function () {
    var tokens = lunr.tokenizer('bar bar', { 'hurp': 'durp' })

...
```
declare module "bar" { declare module.exports: number }
declare module "bar" { declare module.exports: number; }  // also allowed now
```
...
const ast = babylon.parse('var bar = "lol";');
```
...
```js
// `bar` has already been exported. Exported identifiers must be unique. (2:20)
export function bar() {};
export const { a: [{bar}] } = bar;
```

Fix: Check for duplicate named exports in exported rest elements/properties ([#164](https://github.com/babel/babylon/pull/164)) (Kai Cataldo)

```js
// `bar` has already been exported. Exported identifiers must be unique. (2:22)
export const bar = 1;
...
```js
var a : {| x: number, y: string |} = { x: 0, y: 'bar' };
```
...
- Fix parse error with generic methods that have the name `get` or `set` `class bar { get() {} }` ([#55](https://github.com/babel/babylon/pull/55)) @vkurchatkin
...
```js
class Foo {
  constructor(@bar() x, @bar({ a: 123 }) @baz() y) {}
}

export default function func(@bar() x, @bar({ a: 123 }) @baz() y) {}

var obj = {
  method(@bar() x, @bar({ a: 123 }) @baz() y) {}
};
```

...
In the following example, loggers in the `bar.js` file will run under their own configuration, overriding the `package.json` one.
...
function bar() {
  // `barLogger` inherits the config of `signale`
  const barLogger = signale.scope('bar scope');
  ...
  barLogger.success('Hello from the Local scope');
}
...
const barBar = signale.scope('bar', 'bar');

bar.success('bar');
//=> [bar] › ✔  success  bar

barBar.success('bar bar');
//=> [bar] [bar] › ✔  success  bar bar
```
...
//=> [2018-5-15] [11:12:38] [bar.js] › ✔  success  Successful operations
```

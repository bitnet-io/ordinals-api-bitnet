...
/usr/bin/env --debug '-S echo bar bar'
```
...
*   remapping extensions, e.g. so that `import "./bar.js"` will execute `bar.ts`. Currently the following extensions will be mapped:
    *   `.js` to `.ts`, `.tsx`, or `.jsx`
    *   `.cjs` to `.cts`
    *   `.mjs` to `.mts`
    *   `.jsx` to `.tsx`
...
```typescript twoslash
const bar: string | undefined = bar?.bar;
```

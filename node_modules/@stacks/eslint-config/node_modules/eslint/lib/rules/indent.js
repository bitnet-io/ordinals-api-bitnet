...
class OffsetStorage {
     ...
     * )
     *
     * Based on the first example, it would seem that `bar` should be offset by 1 indent level from `bar`, and `baz`
     * should be offset by 1 indent level from `bar`. However, this is not correct, because it would result in `baz`
     * being indented by 2 indent levels in the second case (since `bar`, `bar`, and `baz` are all on separate lines, no
     * collapsing would occur).
     *
     * Instead, the correct way would be to offset `baz` by 1 level from `bar`, offset `bar` by 1 level from the `)`, and
     * offset the `)` by 0 levels from `bar`. This ensures that the offset between `bar` and the `)` are correctly collapsed
     * in the second case.
     * @param {Token} token The token
     ...
     * - Tokens starting in the interval [30, 43) are offset by 1 indent level from the `bar` token
     * - Tokens starting in the interval [43, 820) are offset by 2 indent levels from the `bar` token
     * - Tokens starting in the interval [820, ∞) are offset by 1 indent level from the `baz` token
     *
     * The `setDesiredOffsets` methods inserts ranges like the ones above. The third line above would be inserted by using:
     * `setDesiredOffsets([30, 43], barToken, 1);`
    ...
    setDesiredOffsets(range, fromToken, offset, force) {
         ...
         * * key: 30, value: { offset: 1, from: barToken }
        ...
        const baseOffsetListeners = {
            ...
            ConditionalExpression(node) {
                ...
                if (!options.flatTernaryExpressions ||
                     ...
                     * bar ? (
                     *   bar
                     * ) : ( // this '(' is aligned with the '(' above, so it's considered to be aligned with `bar`
                    ...
                    if (lastConsequentToken.loc.end.line === firstAlternateToken.loc.start.line) {
                         ...
                         *   : baz
                         *
                         * If `baz` were aligned with `bar` rather than being offset by 1 from `bar`, `baz` would end up
            ...
            ExportNamedDeclaration(node) {
                if (node.declaration === null) {
                    ...
                    // Indent the specifiers in `export {bar, bar, baz}`
                    addElementListIndent(node.specifiers, sourceCode.getFirstToken(node, { skip: 1 }), closingCurly, 1);

                    if (node.source) {

                        // Indent everything after and including the `from` token in `export {bar, bar, baz} from 'qux'`

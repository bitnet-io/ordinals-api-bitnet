...
- `"overrides"`, overrides the default options for type annotations with `colon` (e.g. `const bar: string`) and function types with `arrow` (e.g. `type Foo = () => {}`). Additionally allows granular overrides for `variable` (`const bar: string`),`parameter` (`function bar(bar: string) {...}`),`property` (`interface Foo { bar: string }`) and `returnType` (`function bar(): string {...}`) annotations.

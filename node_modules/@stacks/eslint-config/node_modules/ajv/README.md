...
var schema = {
  ...
  "type": "object",
  "properties": {
    "bar": { "$ref": "defs.json#/definitions/int" },
...
console.log(data); // { "bar": 0, "bar": { "baz": "abc", "additional2": 2 }
```
...
The intention of the schema above is to allow objects with either the string property "bar" or the integer property "bar", but not with both and not with any other properties.

With the option `removeAdditional: true` the validation will pass for the object `{ "bar": "abc"}` but will fail for the object `{"bar": 1}`. It happens because while the first subschema in `oneOf` is validated, the property `bar` is removed because it is an additional property according to the standard (because it is not included in `properties` keyword in the same schema).
...
console.log(validate(data)); // true
console.log(data); // { "bar": 1, "bar": "baz" }
```
...
var data = { "bar": "1", "bar": "false" };

var validate = ajv.compile(schema);

console.log(validate(data)); // true
console.log(data); // { "bar": 1, "bar": false }
```
